...
import cn.chason678.mqtt.log.handler.LogService;
import cn.chason678.mqtt.log.handler.impl.LogServiceImpl;
import cn.chason678.mqtt.moquette.proto.messages.*;
import cn.chason678.mqtt.parser.MQTTDecoder;
import cn.chason678.mqtt.parser.MQTTEncoder;
import cn.chason678.mqtt.persistence.Subscription;
import cn.chason678.mqtt.security.AuthorizationClient;
...
public class MQTTSession implements Handler<Message<Buffer>> {
    ...
    public void createInitProcessInstance() {
        ...
        pubTimerID = vertx.setPeriodic(30000, r ->{
            PublishMessage publishMessage = new PublishMessage();
            publishMessage.setTopicName("/" + clientID);
            try {
                publishMessage.setPayload(ByteBuffer.wrap("{\"msgId\":666,\"func\": \"shareDevice\",\"type\": 2,\"gwId\": \"GW01191910010\",\"deviceId\": \"ZG01191910010\",\"adminUid\": \"5c4fe492dc93897aa7d8600b\",\"userAccount\": \"8618954359823\",\"userNickname\": \"小王吧\",\"shareFlag\": 1}".getBytes("UTF-8")));
            }catch (Exception e){
                e.printStackTrace();
            }
            ...
            sendMessageToClient(publishMessage);
        });
        pingTimerID = vertx.setPeriodic(3000, r ->{
            PingReqMessage pingReqMessage = new PingReqMessage();
            if(messageID >= 65535)
                messageID = 0;

            sendMessageToClient(pingReqMessage);
        });
    }
...
//        });
     ...
     */
    @SuppressWarnings("Duplicates")
    public void defaultWriteLogRetry_bak(PublishMessage publishMessage, boolean retryFlag, Handler<AsyncResult<Boolean>> handler) {
...
//                    });
    ...
    /**
     * write mqtt persistence message into mongodb. 
     *
     * @param publishMessage need to stored message
     * @param retryFlag
...
//        logger.info("store publish message into sqlite , clientId -> {} , msgId -> {},topicName -> {}", clientID, publishMessage.getMessageID(),publishMessage.getTopicName());
//        MongoFactory.INSTANCE.sendMessage(new JsonObject().put("timestamp",System.currentTimeMillis())
//                .put("clientId",clientID)
//                .put("messageType",publishMessage.getMessageType())
//                .put("messageId",publishMessage.getMessageID())
//                .put("payload",publishMessage.getPayloadAsString())
//                .put("qos",QOSConvertUtils.toInt(publishMessage.getQos()))
//                .put("topicFilter",publishMessage.getTopicName()),vertx,"insert");
...
//        String topic = pm.getTopicName();
     ...
     * @version 1.0
     */
    public void consumLog_bak(int msgId, Handler<AsyncResult<Long>> handler) {
        if (this.logService != null)
            this.logService.consumLog(msgId, handler);
    }

    /**
     * remove mqtt persistence msg from mongo by {clientId,msgType,msgId}
     * @param msgId
     * @param handler
     */
    public void consumLog(int msgId, Handler<AsyncResult<Long>> handler) {
//        MongoFactory.INSTANCE.sendMessage(new JsonObject().put("clientId",clientID).put("messageType",3).put("messageId",msgId),vertx,"delete");
    }
...
//        deliveryOptions.addHeader("qos", QOSConvertUtils.toStr(publishMessage.getQos()));
        deliveryOptions.addHeader("ip", netSocket.remoteAddress().host());
        if (Objects.nonNull(publishMessage.getMessageID())) {
    ...
    public void sendMessageToClient(AbstractMessage message) {
        try {
            if (message instanceof PublishMessage) {
                // 日志埋点
                logger.info("mqtt-process:" + new JsonObject().put("to","server").put("clientId",clientID).put("msgType", message.getMessageType()).put("ip",netSocket.remoteAddress().host())
