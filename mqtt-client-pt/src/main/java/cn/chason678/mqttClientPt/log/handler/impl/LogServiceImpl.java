package cn.chason678.mqttClientPt.log.handler.impl;
...
//        }
    ...
    @SuppressWarnings("Duplicates")
    @Override
    public void delLog(Handler<AsyncResult<Boolean>> handler){
        logger.debug("del log , clientId -> {}", clientId);
        if (this.stateFile != null)
            count.set(0);
            this.stateFile.write(Buffer.buffer(2).appendShort((short)0), 2, writeRes -> {
                if (writeRes.failed())
                    logger.error(writeRes.cause().getMessage(), "del stateFile fail , clientId -> " + clientId, writeRes);
            });
    };
...
//                                                                                .addHeader("uid", clientId.replace("_",":")));
                                                        }
                                                    });
                                                    endIndex.incrementAndGet();
                                                    processFlag(endIndex, count, msgId, max);
